...
function _fmtMoney(n, cur = "USD") {
  ...
  const x = safeNum(n, 0);
  return x.toLocaleString(locale, { style: "currency", currency: cur, minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
...
export default function Liabilities() {
  ...
  return (
    <div className="p-4 text-slate-300">
      <div className="mb-4">
        <PageHeader title="Liabilities" icon={PageIcons.liabilities}>
          <select
            value={country}
      ...
      {canWrite && showForm && (
        <FormModal title={editingId ? "Edit Liability" : "Add Liability"} onClose={() => resetForm({ hide: true })} wide>
          {error && (
      ...
      {canWrite && deleteTarget && (
        <DeleteConfirmModal
          title={`Delete — ${deleteTarget.description || "Liability Record"}`}
          message="This will permanently delete this liability record. This cannot be undone."
        ...
        >
          ...
          <div className="flex justify-between gap-3">
            ...
            <span>{formatMoney(deleteTarget.value)}</span>
          </div>
        </DeleteConfirmModal>
      )}
